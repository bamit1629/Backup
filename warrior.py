...
def list_users_command(message):
    ...
    if all_users.count() == 0:
        bot.send_message(chat_id, "No users found in the database.", parse_mode='Markdown')
        return

    # Send user details in chunks to avoid Telegram's message limit
    chunk_size = 50
    user_chunks = [all_users[i:i + chunk_size] for i in range(0, all_users.count(), chunk_size)]

    for chunk in user_chunks:
        user_info = ""
        for user in chunk:
            user_info += f"User ID: {user['user_id']}\nPlan: {user['plan']}\nValid Until: {user.get('valid_until', 'N/A')}\n\n"
        
        bot.send_message(chat_id, f"```\n{user_info}\n```", parse_mode='MarkdownV2')

    bot.send_message(chat_id, "*End of user list.*", parse_mode='Markdown')
