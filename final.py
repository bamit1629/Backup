...
@bot.message_handler(commands=['approve', 'remove'])
def approve_or_remove_user(message):
    ...
    if len(cmd_parts) < 2:
        bot.send_message(chat_id, "*Invalid command format. Use /approve <user_id> <plan> <days> or /remove <user_id>.*", parse_mode='Markdown')
        return
    ...
    if action == '/approve':
        ...
        msg_text = f"*User {target_user_id} removed and reverted to free.*"
...
def attack_command(message):
    ...
    try:
        user_data = users_collection.find_one({"user_id": user_id})
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*You are not approved to use this bot. Please contact@WarriorsLab & @iconic00001.*", parse_mode='Markdown')
            return
...
def handle_message(message):
    if message.text == "Instant Plan 🧡":
        ...
        bot.send_message(message.chat.id, "*Please use the following link for Setup Process: https://t.me/+gOMenhEpAwQ2Nzg1 *", parse_mode='Markdown')
        ...
        if user_data:
            ...
            response = "*No account information found. Please contact@WarriorsLab & @iconic00001.*"
        bot.reply_to(message, response, parse_mode='Markdown')
    elif message.text == "Rules❓":
        bot.reply_to(message, "*1. MUST FOLLOW RULES OTHERWISE YOU WILL KICKED BY OWNER*",parse_mode = 'Markdown'
        bot.reply_to(message, "*2. Min time 240 & Max time 300 seconds*",parse_mode = 'Markdown'
        bot.reply_to(message, "*3. Dont Run Too Many Attacks !! Cause A Ban From Bot*",parse_mode = 'Markdown'
        bot.reply_to(message, "*4. Dont run every match[Use it alternatively 1 match play use it and 2nd match dont use it.]*",parse_mode = 'Markdown'
        bot.reply_to(message, "*5. Dont Run 2 Attacks At Same Time Becz If U Then U Got Banned From Bot.*",parse_mode = 'Markdown'
        bot.reply_to(message, "*6. We Daily Checks The Logs So Follow these rules to avoid Ban!!*",parse_mode = 'Markdown'
