    ...
    if action == '/approve':
        if plan == 1:  # Testing Plan 🧡
            if users_collection.count_documents({"plan": 1}) >= 99:
                bot.send_message(chat_id, "*Approval failed: Testing Plan 🧡 limit reached (99 users).*", parse_mode='Markdown')
                return
...
def handle_message(message):
    if message.text == "Testing Plan 🧡":
        ...
        if user_data:
            ...
            response = "*No account information found. Please contact @Warriors_King & @Iconic_Hack.*"
